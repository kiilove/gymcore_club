...
const FormClasses = ({ classesData, onSave }) => {
  ...
  const changeIsAcitve = (parentTitle, parentKey, childrenKey, data = []) => {
    console.log({ parentTitle, parentKey, childrenKey, data, classesData });
    const findGroup = classesData[parentTitle];
    console.log(findGroup);
  };

  const renderClassItem = (data = [], parentKey, parentTitle) => {
    if (data.length === 0) {
      return;
    }
          ...
          extra={
            <div className="flex gap-x-2 justify-center items-center">
              ...
              <Switch
                ...
                checked={isActive}
                onClick={() =>
                  changeIsAcitve(parentTitle, parentKey, childrenKey, data)
                }
              />
            </div>
          }
  ...
  useEffect(() => {
    ...
    const tabsItems = classesData[currentGroupTitle]?.map((item, idx) => {
      const { title: label, id: key, classes } = item;
      const children = renderClassItem(classes, key, currentGroupTitle);
